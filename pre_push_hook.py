...
BACKEND_URL     = os.environ.get("ARCH_BACKEND_URL", DIAGRAM_GENERATOR_URL).strip()
BACKEND_MODEL   = os.environ.get("ARCH_BACKEND_MODEL", "fabcric-final:latest").strip()
...
def main():
    ...
    llm_result = None
    print(f"[arch-hook] Incerc diagram_generator @ {BACKEND_URL}...")

    try:
        llm_result = refine_with_llm(
            static_result,
            diagram_generator_url=BACKEND_URL,
            diagram_generator_model=BACKEND_MODEL,
