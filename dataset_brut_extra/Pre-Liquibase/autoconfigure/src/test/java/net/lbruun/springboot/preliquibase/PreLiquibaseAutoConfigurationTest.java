...
public class PreLiquibaseAutoConfigurationTest {
      ...
      new ApplicationContextRunner()
          .withConfiguration(
              AutoConfigurations.of(
                  PreLiquibaseAutoConfiguration.class, LiquibaseAutoConfiguration.class))
            ...
            assertPreLiquibase(
                (preLiquibase) -> {
                  ...
                  assertThat(getScriptFileName(preLiquibase, 0))
                      .endsWith("preliquibase/hsqldb.sql");
                }));
  }
  ...
  void backsOffIfNotEnabledLiquibase() {
    this.contextRunner
        .withUserConfiguration(EmbeddedDataSourceConfiguration.class)
        .withPropertyValues("spring.liquibase.enabled=false", "spring.datasource.url=" + JDBC_URL1)
        .run((context) -> assertThat(context).doesNotHaveBean(PreLiquibase.class));
  }
  ...
  void backsOffIfCustomDefinedPreLiquibase() {
    this.contextRunner
        .withUserConfiguration(
            EmbeddedDataSourceConfiguration.class, PreLiquibaseUserConfiguration.class)
        .withPropertyValues("sql.script.schemaname=myschema", "spring.datasource.url=" + JDBC_URL1)
        .run(
            (context) -> {
        ...
        .run(
            assertPreLiquibase(
                (preLiquibase) -> {
                  ...
                  assertThat(getScriptFileName(preLiquibase, 1))
                      .endsWith("preliquibase-customlocation/default.sql");
                }));
  }
