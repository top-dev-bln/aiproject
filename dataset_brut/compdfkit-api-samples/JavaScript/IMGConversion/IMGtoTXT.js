...
document.getElementById('uploadForm').addEventListener('submit', async function (event) {
  ...
  // 2.Create Task
  const createTaskResponse = await fetch('https://api-server.compdf.com/server/v1/task/img/txt?language=2', {
    method: 'GET',
    headers: {
  ...
  pdfData.append('parameter', '{    "isAllowOcr":1,"isContainOcrBg":0 }');
