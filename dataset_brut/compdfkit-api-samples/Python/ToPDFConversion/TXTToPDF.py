...
# 2. Create Task
create_task_response = requests.get(CREATE_TASK_URL + "/txt/pdf?language=2", headers={"Authorization": bearer_token})
...
with open(pdf_path, "rb") as file:
    files = {
        'file': ('txt.txt', file, 'application/octet-stream'),
    }
    upload_payload = {
        "taskId": task_id,
        "language": "2"
