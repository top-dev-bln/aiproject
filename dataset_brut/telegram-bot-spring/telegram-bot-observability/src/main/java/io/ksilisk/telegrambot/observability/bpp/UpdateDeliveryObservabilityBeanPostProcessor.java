package io.ksilisk.telegrambot.observability.bpp;

import io.ksilisk.telegrambot.core.delivery.UpdateDelivery;
import io.ksilisk.telegrambot.observability.marker.ObservabilityWrapped;
import io.ksilisk.telegrambot.observability.recorder.MetricsRecorder;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.aop.framework.ProxyFactory;
import org.springframework.beans.BeansException;
import org.springframework.beans.factory.ObjectProvider;
import org.springframework.beans.factory.config.BeanPostProcessor;
import org.springframework.core.Ordered;
import org.springframework.core.PriorityOrdered;
import org.springframework.util.ClassUtils;

import java.lang.reflect.Method;
import java.lang.reflect.Modifier;

/**
 * BeanPostProcessor that instruments {@link UpdateDelivery}
 * to record delivery-level metrics.
 */
public final class UpdateDeliveryObservabilityBeanPostProcessor implements BeanPostProcessor, PriorityOrdered {
    private static final Logger log = LoggerFactory.getLogger(UpdateDeliveryObservabilityBeanPostProcessor.class);

    private final MetricsRecorderResolver metricsRecorderResolver;

    public UpdateDeliveryObservabilityBeanPostProcessor(ObjectProvider<MetricsRecorder> recorderProvider) {
        this.metricsRecorderResolver = new MetricsRecorderResolver(recorderProvider, log);
    }

    @Override
    public int getOrder() {
        return Ordered.LOWEST_PRECEDENCE;
    }

    @Override
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (!(bean instanceof UpdateDelivery)) {
            return bean;
        }
        if (bean instanceof ObservabilityWrapped) {
            return bean;
        }

        Class<?> userClass = ClassUtils.getUserClass(bean);

        boolean finalClass = Modifier.isFinal(userClass.getModifiers());
        boolean finalDeliver = isFinalExecuteMethod(userClass);

        if (finalClass || finalDeliver) {
            log.warn("Observability: UpdateDelivery bean '{}' ({}) is final (classFinal={}, deliverFinal={}); " +
                            "metrics instrumentation skipped.",
                    beanName, userClass.getName(), finalClass, finalDeliver);
            return bean;
        }

        MetricsRecorder metrics = metricsRecorderResolver.get();

        ProxyFactory pf = new ProxyFactory();
        pf.setTarget(bean);
        pf.setProxyTargetClass(true);
        pf.setInterfaces(ProxyInterfaces.mergeInterfacesDeep(userClass, ObservabilityWrapped.class));
        pf.addAdvice(new UpdateDeliveryMetricsInterceptor(metrics));

        return pf.getProxy();
    }

    private static boolean isFinalExecuteMethod(Class<?> userClass) {
        for (Method m : userClass.getMethods()) {
            if (!"deliver".equals(m.getName())) {
                continue;
            }
            if (m.getParameterCount() != 1) {
                continue;
            }
            if (Modifier.isFinal(m.getModifiers())) {
                return true;
            }
        }
        return false;
    }
}
